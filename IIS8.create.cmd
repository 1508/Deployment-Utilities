...
echo   IIS8.CREATE "%WEBROOT%" "%HOSTHEADER%" "%WEBNAME%" "%APPNAME%" "%ASPNETFRAMEWORK_REGIIS_PATH%" "%managedPipelineMode%" "%bin%"
echo. 

:Step1
echo Performing step 1
rem Windows 8 and Windows Server 2012 are using same deployment script for IIS8 setup.
...
:Step6ASP40
REM Default setup on IIS8 is a Framework 4.0 setup... 
...
:HELP
echo.
echo   Creates IIS8 Website and App Pool
echo.
echo   IIS8.CREATE webroot hostheader webname appname aspnetframework_regiis_path managedPipelineMode bin
echo.
